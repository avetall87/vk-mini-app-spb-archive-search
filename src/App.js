...
const App = () => {
  ...
  useEffect(() => {
    ...
    async function fetchLaunchParameters() {
      ...
      if (splitData &&
          splitData.filter(value => value.includes('vk_viewer_group_role')).
              map(value => value.split('=')[1])[0] === 'admin') {
        setCommunityAdmin(true);
      }

      const fetchData = await splitData.filter(
          value => value.includes('vk_group_id')).
          map(value => value.split('=')[1]);
      const groupId = fetchData[0];
      setVkGroupId(groupId);

      const fetchAppId = await splitData.filter(
          value => value.includes('vk_app_id')).
          map(value => value.split('=')[1]);
